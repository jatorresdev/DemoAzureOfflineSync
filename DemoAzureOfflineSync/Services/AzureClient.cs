...
namespace DemoAzureOfflineSync.Services
{
    public class AzureClient
    {
        ...
        private const string serviceUri = "http://intermedio-mod4mobiledevops.azurewebsites.net/";
        ...
        public async Task SyncAsync(bool ignoreConnectivity)
        {
            ...
            else
            {
                if (!Plugin.Connectivity.CrossConnectivity.Current.IsConnected)
                {
                    ...
                    {
                        await App.Current.MainPage.DisplayAlert("DemoAzureOfflineSync", "Es necesario que actives tu red para sincronizar información.", "Aceptar");
                    });
                }
                else
                {
        ...
        private async Task ProcessSync()
        {
            ...
            try
            {
                await _table.Where(t => t.IsLocal).ToListAsync().ContinueWith(async (b) =>
                {
        ...
        {
            await _table.PurgeAsync("allContacts", string.Empty, true, new System.Threading.CancellationToken())
                .ContinueWith((b) => { SetNotification(GetPendingOperations()); });
        }

        public string GetPendingOperations()
        {
            return string.Format("Operaciones Pendientes Por Enviar: {0}", _client.SyncContext.PendingOperations.ToString());
        }
